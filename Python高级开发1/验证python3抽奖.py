...
def read_students_excel():
    ...
    for i in range(1, rows):
        s_id = sheet.cell(i, 0).value.strip() #去除首尾空格
        ...
        if not (s_id and s_name and s_class):
            continue  # 跳过不完整的行
        tmp_str = "%s | %s | %s" % (s_id, s_name, s_class) #按照指定格式输出结果
        all_s.append(tmp_str) #结果追加到列表中
        #print(all_s)
    return all_s
def get_real_t():
    data_all = xlrd.open_workbook(excel_path, formatting_info=True)
...
def get_labels(nbr):
    ...
    for idx in range(0, nbr):
        ...
        tmp_label.place(x=30, y=90+idx*int(380/nbr), width=440, height=int(380/nbr))  #设置多个间距相等，大小相等的标签
        labels.append(tmp_label)
    return labels  #返回这个列表
...
def show_history():
    ...
    for idx in range(nbr):
        list_label[idx]['text'] = already_tmp[idx]  #创建bian'qian
